...
public class Model extends Observable {
    ...
    public void tilt(Side side) {
        ...
        for (int c = 0; c <_board.size(); c++) {
            int sCol = c;
            int sRow = _board.size() - 1;
            Tile sentinel = _board.tile(sCol, sRow);
            for (int r = _board.size() - 2; r >= 0; r--) {
                ...
                if (sentinel == null) {
                    _board.move(sCol, sRow, t);
                    sentinel = _board.tile(sCol, sRow);
                } else {
                    if (sentinel.value() == t.value()) {
                        /** Update score */
                        _score += 2 * sentinel.value();
                        /** Merge tiles */
                        _board.move(sCol, sRow, t);
                        /** Downward sentinel 1 row */
                        sRow -= 1;
                        sentinel = _board.tile(sCol, sRow);
                    } else {
                        /** Downward sentinel 1 row */
                        sRow -= 1;
                        sentinel = _board.tile(sCol, sRow);
                        /** If the new sentinel's position is empty,
                         * merge t to the new position */
                        if (sentinel == null) {
                            _board.move(sCol, sRow, t);
                            sentinel = _board.tile(sCol, sRow);
                        }
    ...
    public static boolean atLeastOneMoveExists(Board b) {
        ...
        /** Check if there are any two adjacent tiles with the same value on the same row. */
        for (int r = 0; r < b.size(); r++) {
        ...
        /** Check if there are any two adjacent tiles with the same value on the same column. */
        for (int c = 0; c < b.size(); c++) {
